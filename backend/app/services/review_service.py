...
def review_draft_with_metrics(
    ...
    style_score = max(0.0, 1.0 - (len(style_items) * 0.12))
...
def revise_draft(content_md: str, issues: list[dict[str, Any]]) -> str:
    ...
    for issue in issues:
        ...
        if severity == "high" and issue_type in {"evidence", "citation"}:
            drop_indices.add(paragraph_index)
        else:
    ...
    for block in blocks:
        ...
        claim_counter += 1
        if claim_counter in drop_indices:
            continue
    ...
    revised += "\n\n> 修订说明：已基于审校意见进行定向修订，高风险段落已移除，事实与表达问题已修正。"
    return revised
